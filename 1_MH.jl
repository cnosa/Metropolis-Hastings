...
# ╔═╡ c6fbb510-b058-11ef-0fed-8f8ad0740745
using PlutoUI

# ╔═╡ cf61c16e-4e03-4737-9484-fe2134cb81cb
begin
	using Plots
...
# ╔═╡ 8b28a191-1733-44e0-8605-0e46338c37a4
PlutoUI.TableOfContents(title="MCMC methods: Metropolis Hastings, Gibbs sampler and Two-walk", aside=true)

# ╔═╡ 299cd15a-af59-4a97-810d-99bd15bd2375
md"""
...
# ╔═╡ 65abbb76-3e5c-46a8-ac7a-1c28f3bedf33
...
# ╔═╡ 08ddedb6-2321-492b-8104-b28f594d6418
function MCIntegration(s)
    ...
    rate = [ones(n-1) log.(1:n-1) ] \ log.(convergence .+ 1E-8)
    
    return convergence, rate
end

# ╔═╡ bcd7f5f2-9181-4cb2-9b67-62184f537c6e
md"""
# Example 0
"""

# ╔═╡ b86797fa-b25e-489b-a61c-aeca0736f544
begin
	function energia0(p)
	    x = p[1]
	    y = p[2]
...
# ╔═╡ 54cbe529-d268-4ee7-a1c9-a46efad20717
md"""
## Using MHRW
"""

# ╔═╡ 9c565b7d-28b6-42e2-919d-bd20f650e2bf
samples0, probabilities0, MAP0, CM0, rateAlpha0 = MHRW(energia0,2,[-15.0,-15.0],0.1,10000)

# ╔═╡ bb352213-0ca0-4577-bdb6-5732bee40f49
begin
	xhist = samples0[1, :]
	yhist = samples0[2, :]
...
# ╔═╡ b872bc7c-a1d3-4774-995d-8b64ae49fefe
begin
...
# ╔═╡ 874f3097-2dec-49a5-a8b5-2223e536142d
begin
...
# ╔═╡ 3eaab2c5-c2a0-4a77-a41b-bd97f7766a1f
md"""
# Example 1
"""

# ╔═╡ c5347a37-8c9e-46c6-ad63-65ef8d2c1e65
begin
	function energia1(p)
	    x = p[1]
	    y = p[2]
	...
	ran1 = range(-20, 20, length=100)
	contour1 = contour(ran1, ran1, [energia1((i, j)) for j in ran1, i in ran1], levels=100, linewidth=2, color=:reds)
end

# ╔═╡ 7647a285-4ec0-4191-87b0-325cca2bf51b
md"""
## Using MHRW
"""

# ╔═╡ e870f521-cc8e-4bdb-84be-25955abc6151
samples1, probabilities1, Alpha1, MAP1, CM1 = MHRW(energia1,2,[20.0,-20.0],0.5,10000)

# ╔═╡ 8037f401-40d2-41c7-a1a3-7279efd5108a
function pointcloud(delta,iterations=10000)
    ...
    sxy = contour(ran1, ran1, [energia1((i, j)) for j in ran, i in ran], levels=30, linewidth=2, color=:reds,xlim=(-lim,lim),ylim=(-lim,lim),
    colorbar=false)
...
# ╔═╡ b522355d-72ce-46d1-82c0-f1eec54a5091
begin
	p111 = plot(pointcloud(0.01))
	p121 = plot(pointcloud(0.25))
	p211 = plot(pointcloud(0.5))
	p221 = plot(pointcloud(1))
	
	plot(p111,p121,p211,p221,layout=(2,2))
	
end

# ╔═╡ ee544089-ed25-4edf-bb31-815a67919359
md"""
# Example 2
"""

# ╔═╡ c10a9657-f042-4343-8af1-5cadc26727d2
begin
	function energia2(p)
	    x = p[1]
	    y = p[2]
	...
	ran2 = range(-15, 15, length=100)
	contour2 = contour(ran2, ran2, [energia2((i, j)) for j in ran2, i in ran2], levels=20, linewidth=2, color=:RdBu, xlabel="x",ylabel="y", title="Contour plot of exp(f)")
end

# ╔═╡ fc351c78-8d22-4d64-af7e-948d909ca281
md"""
## Using MH
"""

# ╔═╡ 06123082-ce9d-4ae8-b6a7-302501d850ce
samples2, probabilities2, Alpha2, MAP2, CM2 = MHRW(energia2,2,[-10.0,-10.0],0.2,50000)

# ╔═╡ ea9825db-7085-45b0-9e5a-5ed261a9842f
begin
	xhist2 = samples2[1, :]
	yhist2 = samples2[2, :]
	lim2 = 15
	sxy2 = contour(ran2, ran2, [energia2((i, j)) for j in ran2, i in ran2], levels=100, linewidth=1, color=:RdBu,xlim=(-lim2,lim2),ylim=(-lim2,lim2))
	sxy2 = scatter!(xhist2[1:10:end], yhist2[1:10:end], markersize=2, title="Point cloud", color="black", alpha=0.5,
	    xlabel="x", ylabel="y", border=:box,xlim=(-lim2,lim2),ylim=(-lim2,lim2),colorbar=false)
	hx2 = histogram(xhist2,color=:dodgerblue,xlim=(-lim2,lim2))
	hy2 = histogram(yhist2, orientation=:horizontal,ylim=(-lim2,lim2),color=:lightcoral)
	p02 = Plots.Plot()
	cuadricula2 = grid(2,2, heights=(0.2,0.8), widths=(0.8,0.2))
	plot(hx2, p02, sxy2, hy2, legend=:none, layout=cuadricula2,size=(800,600))
	
end

# ╔═╡ 97a04c80-1d3c-41dc-9ed4-ee7169791453
md"""
## Using Gibbs sampler
"""

# ╔═╡ 93ca914b-926a-4299-81e5-266ed13826c4
begin
	N = 10000
	x = zeros(N+1)
	y = zeros(N+1)
...
# ╔═╡ bcbf0d81-7f4e-4820-bb97-3762ce624144
begin
...
# ╔═╡ e42a64f1-f014-4af5-b0d9-4c2a2c0c6ca6
md"""
## Using two-walk
"""

# ╔═╡ 78061ed6-0f70-45c5-bb21-249c2f82b6e4
...
# ╔═╡ e1717726-5121-4155-9a94-777149cc5597
md"""
## Comparison of methods
"""

# ╔═╡ bf549ad8-24cd-447d-8e82-be0c82006348
begin
...
# ╔═╡ df676602-c0b9-47c4-a7b3-457429ce6c8d
Conv = [MHx[2][2] MHy[2][2];
    Gibbsx[2][2] Gibbsy[2][2];
    twowalkx[2][2] twowalky[2][2] ]

# ╔═╡ 1aada0a3-0c42-439e-8f40-d7e068a7427a
begin
	lim = 15
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "8a2f04c5e437a5bb729c1fcd3d04902ab4e86328"
